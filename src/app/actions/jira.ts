...
export async function createIssueInJira(ticketData: any, jiraProjectId: string, jiraIssueTypeId: string, additionalFields: Record<string, any> = {}) {
  ...
  rawContent = rawContent.replace(/!\[(.*?)\]\((data:image\/[^;]+;base64,[^\)]+)\)/g, (match: string, alt: string, dataUri: string) => {
  ...
  let rawImageIndex = 1;
  rawContent = rawContent.replace(/(data:image\/[^;]+;base64,[^\s"'\)]+)/g, (match: string) => {
