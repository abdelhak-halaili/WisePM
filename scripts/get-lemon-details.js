...
async function fetchLemonData() {
  ...
  try {
      ...
      if (!productsData.included) {
         console.log('No "included" field found. Checking if variants are inside data...');
      }
      
      // Flatten variants from "included" array
      const variants = productsData.included ? productsData.included.filter((item) => item.type === 'variants') : [];
