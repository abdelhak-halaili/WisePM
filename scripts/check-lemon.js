...
async function checkLemon() {
    try {
        ...
        console.log('\n--- Fetching Stores ---');
        const storesResponse = await axios.get('https://api.lemonsqueezy.com/v1/stores', {
            headers: {
                'Accept': 'application/vnd.api+json',
                'Content-Type': 'application/vnd.api+json',
                'Authorization': `Bearer ${apiKey}`
        ...
        storesResponse.data.data.forEach(s => {
            console.log(`Store ID: ${s.id} | Name: ${s.attributes.name}`);
        });
