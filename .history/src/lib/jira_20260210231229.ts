...
async function jiraRequest(endpoint: string, accessToken: string, cloudId: string, options: RequestInit = {}) {
  ...
  if (!response.ok) {
    ...
    try {
        ...
        parsedError = { errorMessages: [errorBody] };
    }

    // specific handling for 400 to show validation errors
    if (response.status === 400 && parsedError.errors) {
         // parsedError.errors is a key-value map of field: error
         const fieldErrors = Object.entries(parsedError.errors).map(([key, msg]) => `${key}: ${msg}`).join(', ');
         throw new Error(`Validation Failed: ${fieldErrors}`);
    }

    if (parsedError.errorMessages && parsedError.errorMessages.length > 0) {
        throw new Error(`Jira API Error: ${parsedError.errorMessages.join(', ')}`);
    }
...
export const getJiraIssueTypes = async (accessToken: string, cloudId: string, projectId?: string) => {
  if (projectId) {
     // Expand fields to get required fields for dynamic forms
     const data = await jiraRequest(`/rest/api/3/issue/createmeta?projectIds=${projectId}&expand=projects.issuetypes.fields`, accessToken, cloudId);
...
export const uploadJiraAttachment = async (accessToken: string, cloudId: string, issueIdOrKey: string, formData: FormData) => {
  const url = `https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/issue/${issueIdOrKey}/attachments`;
