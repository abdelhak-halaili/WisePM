        ...
        {step === 1 && (
          <>
            ...
            <div className={styles.formGroup}>
              ...
              <div className={styles.typeGrid} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <button
                  className={`${styles.platformCard} ${formData.ticketType === 'functional' ? styles.selected : ''}`}
                  onClick={() => setFormData({ ...formData, ticketType: 'functional' })}
                  style={{ justifyContent: 'center', textAlign: 'center', height: 'auto', padding: '1rem' }}
                >
                  <div style={{ fontWeight: 600, marginBottom: '0.25rem' }}>Functional Ticket</div>
                  <div style={{ fontSize: '0.8rem', opacity: 0.8 }}>For PMs & Stakeholders (User Stories)</div>
                </button>
                <button
                  className={`${styles.platformCard} ${formData.ticketType === 'technical' ? styles.selected : ''}`}
                  onClick={() => setFormData({ ...formData, ticketType: 'technical' })}
                  style={{ justifyContent: 'center', textAlign: 'center', height: 'auto', padding: '1rem' }}
                >
                   <div style={{ fontWeight: 600, marginBottom: '0.25rem' }}>Technical Ticket</div>
                   <div style={{ fontSize: '0.8rem', opacity: 0.8 }}>For Engineers (Implementation/Arch)</div>
                </button>
              </div>
            </div>
