...
import { generateTicketAction, reformTextAction, saveTicketAction, refineTicketAction, listProjectsAction, createProjectAction, checkUserSubscriptionAction } from '@/app/dashboard/tickets/actions'
...
export default function TicketWizard() {
  ...
  const handleJiraExport = async () => {
    const isPro = await checkUserSubscriptionAction();
    if (isPro) {
        setShowJiraModal(true)
    } else {
        setLimitMessage("Integration with Jira is a Pro feature. Upgrade to sync your tickets seamlessly.");
