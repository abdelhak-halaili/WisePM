    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            requiredFields.forEach(f => {
                ...
                // If it's an object with ID (for selects), strictly pass { id: ... }
                // If it's a string, pass string.
                // Minimal mapping logic:
                if (f.schema?.type === 'array' || f.schema?.items) {
                    // Arrays handling (e.g. Components)
                    // Simplified: if value is {id: ...}, wrap in array
                    if (val && val.id) {
            ...
            const result = await createIssueInJira(generatedResult, selectedProject, selectedType, additionalFields)
            
            if (result.success && result.data) {
                setCreatedKey(result.data.key)
                setCreatedLink(result.data.self)
            } else {
                throw new Error(result.error || 'Failed to create ticket')
            }
        } catch (e: any) {
            setError(e.message)
            // alert('Failed to create issue: ' + e.message) // Error is shown in UI now
        } finally {
            setIsCreating(false)
        }
    }
