...
export default function TicketWizard() {
  ...
  const JiraModal = () => {
    ...
    const handleCreate = async () => {
        setIsCreating(true)
        try {
            const raw = await createIssueInJira(generatedResult, selectedProject, selectedType)
            // Result: { id: "...", key: "PROD-123", self: "..." }
            setCreatedKey(raw.key)
            setCreatedLink(raw.self) // Self is API link, we want browser link usually. 
            // Better: use projects[0].self to guess base URL or just use provided link if available.
            // Actually, `self` is the API url. Browser URL is usually `https://[domain]/browse/[KEY]`
            // We can construct it if we have domain, or just show key.
            // Jira Response usually doesn't give browser URL directly.
        ...
        <div className={styles.jiraModal} onClick={e => e.stopPropagation()}>
           ...
           {createdKey ? (
               <div className={styles.jiraForm} style={{ textAlign: 'center', padding: '2rem' }}>
                   ...
                   {/* We might not have the full URL handy easily without passing it through. */}
                   <p>Your ticket has been successfully created in Jira.</p>
                   <button className={styles.buttonPrimary} onClick={() => setShowJiraModal(false)} style={{ marginTop: '1rem' }}>
                       Done
                   </button>
               </div>
           ) : (
               ...
               <div className={styles.jiraActions}>
                 ...
                 <button 
                    ...
                    disabled={isCreating || !selectedProject || !selectedType || !!error}
                 >
