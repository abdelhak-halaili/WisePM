...
import { generateTicketAction, reformTextAction, saveTicketAction, refineTicketAction } from '@/app/dashboard/tickets/actions'
...
export default function TicketWizard() {
  ...
  const [jiraConfig, setJiraConfig] = useState<{domain: string, project: string} | null>(null)
  
  // Load Jira Config
  useEffect(() => {
    const saved = localStorage.getItem('wisepm_jira_config')
    if (saved) {
      try { setJiraConfig(JSON.parse(saved)) } catch (e) {}
    }
  }, [])
  ...
  const handleJiraExport = () => {
    ...
    // Construct Jira URL
    const domain = jiraConfig.domain.replace(/\/$/, '').replace(/https?:\/\//, '')
    const project = jiraConfig.project.toUpperCase()
    const summary = encodeURIComponent(generatedResult?.title || 'New Ticket')
    const description = encodeURIComponent(generatedResult?.coreContent || '')
    
    // URL Scheme: https://[domain]/secure/CreateIssueDetails!init.jspa?pid=[PROJ_ID]&issuetype=[TYPE]&summary=...&description=...
    // Note: PID is tricky if we only have Key. 
    // Fallback: Use simple CreateIssue.jspa?pid=&issuetype=&summary=&description= which might redirect or use easier params if available.
    // Actually, for Project Keys, we often need the project ID (pid). 
    // BUT! /secure/CreateIssue!default.jspa exists too.
    // Let's try the modern Deep Link which is robust:
    // https://[domain]/secure/CreateIssueDetails!init.jspa?pid=[PID] is hard without PID.
    //
    // ALTERNATIVE: Use the "Issue Collector" style or just open the create screen.
    // Hack: If we use /secure/CreateIssue!default.jspa, it opens the form.
    // Pre-filling with Project KEY is supported via `project` param in some versions or `pid`.
    // Let's use the standard "Quick Create" link if possible:
    // https://[domain]/secure/CreateIssueDetails!init.jspa?pid=10000&... (Requires ID)
    // 
    // WAIT! We can't easily get PID from Key without API.
    // Let's accept that we might just open the dashboard or a generic create link if we can't find a better way.
    //
    // BETTER: Use /browse/[PROJECT_KEY] and let them click create? No.
    //
    // LET'S TRY: https://[domain]/secure/CreateIssue!default.jspa
    // Does it take params? Sometimes.
    //
    // OK, the MOST reliable for "Smart Link" without ID is unclear.
    // Let's start with a Prompt to user: "Enter Project Key (e.g. PROD)"
    // If we can't map Key -> ID client-side, we might just open the create form generic.
    //
    // ACTUALLY: New Jira Cloud URL:
    // https://[domain]/browse/[PROJECT_KEY]?selectedIssue=[KEY] (View)
    // 
    // Let's stick to the Classic URL but warn user they might need to select Project.
    // Fallback strategy: Copy content to clipboard and open Jira.
    // Jira URLs are notoriously difficult without Project ID/IssueType ID.
    // The "CreateIssueDetails!init.jspa" endpoint often requires IDs to work properly.
    
    // 1. Copy Description to Clipboard
    const fullContent = generatedResult?.coreContent || ''
    navigator.clipboard.writeText(fullContent).then(() => {
        // 2. Alert user
        alert(`Content copied to clipboard! Paste it into the Description field in Jira.\n\nOpening Jira now...`)
        
        // 3. Open generic Create Issue form
        // Trying to pass summary at least, as it sometimes works.
        const url = `https://${domain}/secure/CreateIssue!default.jspa?summary=${summary}`
        window.open(url, '_blank')
    }).catch(err => {
        console.error('Failed to copy text: ', err)
        // Fallback open without copy
         window.open(`https://${domain}/secure/CreateIssue!default.jspa`, '_blank')
    })
  }

  const saveJiraConfig = (domain: string, project: string) => {
    const config = { domain, project }
    setJiraConfig(config)
    localStorage.setItem('wisepm_jira_config', JSON.stringify(config))
    setShowJiraModal(false)
    // Re-trigger export immediately? No, let them click again to be safe.
  }
  ...
  // Jira Config Modal
  const JiraModal = () => {
    if (!showJiraModal || !mounted) return null
    
    // Local state for inputs
    const [localDomain, setLocalDomain] = useState(jiraConfig?.domain || '')
    const [localProject, setLocalProject] = useState(jiraConfig?.project || '')

    return createPortal(
      <div className={styles.lightboxOverlay} onClick={() => setShowJiraModal(false)}>
        <div className={styles.jiraModal} onClick={e => e.stopPropagation()}>
           <div className={styles.jiraHeader}>
             ...
             <button className={styles.buttonGhost} onClick={() => setShowJiraModal(false)}>
               <X size={20} />
             </button>
           </div>
           
           <div className={styles.jiraForm}>
             <div>
               <label className={styles.jiraLabel}>Jira Domain</label>
               <input 
             ...
             <div>
               <label className={styles.jiraLabel}>Default Project Key (Optional)</label>
               <input 
                 className={styles.jiraInput} 
           ...
           <div className={styles.jiraActions}>
             <button className={styles.jiraCancelBtn} onClick={() => setShowJiraModal(false)}>Cancel</button>
             <button 
                className={styles.jiraSaveBtn} 
                onClick={() => saveJiraConfig(localDomain, localProject)}
        ...
        <div className={styles.actions}>
          ...
          <button 
            ...
            disabled={
               ...
               (step === 2 && (!formData.problem || !formData.behavior || formData.screenshots.some(s => !s.description.trim()))) ||
