...
export default function RichTextEditor({ content, onChange, editable = true, placeholder = 'Start writing...' }: RichTextEditorProps) {
  // Jira-like behavior: Force specific block separation
  const formattedContent = React.useMemo(() => {
      if (!content) return '';
      // Replace single newlines with double newlines to force Paragraphs instead of Hard Breaks
      // This matches the "Jira" feel where Enter = New Paragraph
  ...
  const editor = useEditor({
    extensions: [
      StarterKit.configure({
        ...
        // We still want Shift+Enter to work as HardBreak, which StarterKit includes
      }),
      Markdown.configure({
        html: false,
        transformPastedText: true,
        transformCopiedText: true,
        breaks: false, // We want explicit paragraphs now, so we disable the "soft break = br" behavior to rely on our pre-processing
      }),
