  ...
  const prompt = `
    ...
    1. **Core Content (coreContent)**: 
       ...
       - **GHERKIN RULES (If Gherkin format is requested)**:
         1. **Single User Story**: Start with ONE unified User Story that covers the whole feature.
            Example: "As a [role], I want to [action], so that [benefit]."
         2. **Scenarios**: Generate as many relevant scenarios as possible (Happy Path, Edge Cases, Error States). Be exhaustive and clear.
         3. **Format**:
            ### Scenario: [Title]
            [[N]] (Image Placeholder)
            Given [Context]
