...
export async function refineTicketAction(
  ...
  const prompt = `
    ...
    2. **IF ACTION (Explicit Edit Request)**:
       - UPDATE the \`updatedTicket\` fields (coreContent, etc.) to reflect the change.
       - Set \`message\` to a short confirmation (e.g., "Done. I've added the offline scenario.").
    ...
    3. **IF DISCUSSION (Question/Brainstorming)**:
       ...
       - Set \`message\` to your detailed answer, suggestion, or explanation. The user can manually copy-paste if they like it.
