...
export async function refineTicketAction(
    ...
    properties: {
      updatedTicket: {
        ...
        properties: {
          ...
          coreContent: { type: SchemaType.STRING, description: "The full markdown content, updated based on request" },
          missingElements: { type: SchemaType.STRING, description: "Updated AI Analysis / Engineering Considerations" }
        },
        required: ["title", "type", "coreContent", "missingElements"]
      },
      message: { type: SchemaType.STRING, description: "A concise, polite explanation of what was changed." }
    },
  ...
  const prompt = `
    ...
    Your goal is to MODIFY the JSON ticket data based on the User's latest request.
    ...
    INSTRUCTIONS:
    1. **ACT**: Do not just say you will do it—ACTUALLY update the \`coreContent\` or \`missingElements\`.
    2. **PRESERVE**: Keep existing details unless asked to change them. Do not summarize or lose clarity.
    3. **INTELLIGENT**: If the user says "Split this into two", you might need to ask for clarification, OR just create the first one and mention it in the message. For now, try to handle it within the single ticket structure if possible (clean textual separation).
    4. **MESSAGE**: In the \`message\` field, be brief (1-2 sentences). E.g. "I've added the offline scenario as requested."
  ...
  try {
    ...
    return { success: false, error: 'Failed to refine ticket.' }
  }
}
