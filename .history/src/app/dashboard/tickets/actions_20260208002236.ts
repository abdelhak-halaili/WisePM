...
export async function generateTicketAction(formData: FormData) {
  ...
  const prompt = `
    ...
    2. **Engineering Considerations (missingElements)**:
       ...
       - **FORMATTING**:
         - Do NOT include a main title (e.g. "Engineering Considerations"). Start directly with the content.
         - Use a simple list format: "- **Topic Name**: Description..."
         - Do not use markdown headers (###) for individual items. Use bold text.
